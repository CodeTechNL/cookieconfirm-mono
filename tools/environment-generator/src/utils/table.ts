...
export type RowStatus = "same" | "diff" | "only-env" | "only-ssm";
...
function hardWrap(str: string, limit = 25): string {
  if (!str) return "";
  const out: string[] = [];
...
export interface PlannedChangeRow {
  key: string;
  from?: string;
  to: string;
...
export function renderPlannedChangesTable(rows: PlannedChangeRow[]): string {
  const table = new Table({
