...
import { renderOverviewTable, renderPlannedChangesTable } from './utils/table.js';
import { promptCompanyName, promptEnvFiles, promptProceed, promptStage, promptRegion, promptSearchTerm, promptSelectVariablesWithChoices, promptNewValue, } from './utils/prompts.js';
import { ENV_FILES } from './config/env-files.js';
import chalk from 'chalk';
function compareMaps(envMap, ssmMap) {
    const keys = new Set([...Object.keys(envMap), ...Object.keys(ssmMap)]);
    const rows = [];
...
function displayPrefixFor(companyName, stage) {
...
async function main() {
    ...
    for (const filePath of envFiles) {
        ...
        const ssmSubset = {};
        for (const k of Object.keys(singleEnv)) {
            if (ssmMap[k] !== undefined)
                ssmSubset[k] = ssmMap[k];
        }
        const perFileRows = compareMaps(singleEnv, ssmSubset);
        const label = labelByPath.get(filePath) ?? filePath;
